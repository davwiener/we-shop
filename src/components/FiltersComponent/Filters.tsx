      ...
      {props.filters.map((filter: filterClass) => (
        <div className="filter" key={`filters-${filter.filterName}`}>
           {filter.filterType === FilterType.Dropdown && (
            <DropdownFilter filter={filter} updateFilter={props.updateFilter}></DropdownFilter>
           )}
           {filter.filterType === FilterType.FreeText && (
            <FreeTextFilter filter={filter} updateFilter={props.updateFilter}></FreeTextFilter>
           )}
           {filter.filterType === FilterType.Date && (
            <DateFilter filter={filter} updateFilter={props.updateFilter}></DateFilter>
           )}
           {filter.filterType === FilterType.Range && (
            <RangeFilter filter={filter as rangeFilter} updateFilter={props.updateFilter}></RangeFilter>
           )}
        </div>
      ))}
    </div>
